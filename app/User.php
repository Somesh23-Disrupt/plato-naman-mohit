...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'name', 'email','inst_name','department','password',
    ];
   ...
   protected $dates = ['trial_ends_at', 'subscription_ends_at'];
    ...
    {   
        ...
        foreach ($users as $user) {
            ...
            if(isset($preferences['user_preferences'])){
                ...
                $cats['name']=$user->name;
                $lmscats ['name']=$user->name;
                $data['cats']=$cats;
                $data['lmscats']=$lmscats;
                $t['quiz']=Quiz::whereIn('category_id',$cats)->where('total_questions','>',0)->get()->count();
                $t['name']=$user->name;
                $data['quiz']=$t;
                $data1[]=$data;
    ...
    {
        $this->attributes['inst_name'] = strtoupper($value);
    }
